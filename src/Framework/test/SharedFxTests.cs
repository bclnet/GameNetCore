...
namespace Microsoft.AspNetCore
{
    public class SharedFxTests
    {
        ...
        public SharedFxTests(ITestOutputHelper output)
        {
            ...
            _sharedFxRoot = Path.Combine(TestData.GetTestDataValue("SharedFrameworkLayoutRoot"), "shared", "Contoso.GameNetCore.App", TestData.GetSharedFxVersion());
        }
        ...
        {
            var runtimeConfigFilePath = Path.Combine(_sharedFxRoot, "Contoso.GameNetCore.App.runtimeconfig.json");

            AssertEx.FileExists(runtimeConfigFilePath);
            AssertEx.FileDoesNotExists(Path.Combine(_sharedFxRoot, "Contoso.GameNetCore.App.runtimeconfig.dev.json"));
        ...
        {
            var depsFilePath = Path.Combine(_sharedFxRoot, "Contoso.GameNetCore.App.deps.json");

            var target = $".NETCoreApp,Version=v{TestData.GetSharedFxVersion().Substring(0, 3)}/{_expectedRid}";
            var ridPackageId = $"Contoso.GameNetCore.App.Runtime.{_expectedRid}";
