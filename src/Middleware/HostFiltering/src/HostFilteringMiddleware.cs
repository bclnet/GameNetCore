...
namespace Contoso.GameNetCore.HostFiltering
{
    /// <summary>
    ...
    {
        // Matches Http.Sys.
        static readonly byte[] DefaultResponse = Encoding.ASCII.GetBytes(
        ...
        readonly ILogger<HostFilteringMiddleware> _logger;
        readonly IOptionsMonitor<HostFilteringOptions> _optionsMonitor;
        HostFilteringOptions _options;
        IList<StringSegment> _allowedHosts;
        bool? _allowAnyNonEmptyHost;
        ...
        IList<StringSegment> EnsureConfigured()
        {
        ...
        bool TryProcessHosts(IEnumerable<string> incoming, IList<StringSegment> results)
        {
        ...
        bool IsTopLevelWildcard(string host) => string.Equals("*", host, StringComparison.Ordinal) // HttpSys wildcard
        ...
        bool CheckHost(HttpContext context, IList<StringSegment> allowedHosts)
        {
